...
export default function CorridorKeyModule() {
  return (
    <ModuleGate moduleId="corridor_key">
      <CorridorKeyInner />
    </ModuleGate>
  );
}

function CorridorKeyInner() {
  const [videoPath, setVideoPath] = useState<string | null>(null);
  const [bgKind, setBgKind] = useState<ChromaBackgroundKind>("color");
  const [bgColor, setBgColor] = useState<string>("#00b140"); // greenscreen-ish
  ...
  // Drop-into-window — only fires when CorridorKey is the active module.
  useModuleDrop("corridor_key", (paths) => {
    const v = paths.find(isVideoPath);
    if (v) setVideoPath(v);
  });

  // Stream backend progress
  useEffect(() => {
    let aborted = false;
  ...
  const run = async () => {
    ...
    try {
      ...
      await notify("Zonthor Studio — фон удалён", name);
    } catch (e) {
      const msg = String(e);
  ...
  return (
    <div className="p-6 grid gap-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div>
            <h1 className="text-lg font-semibold text-zinc-100 flex items-center gap-2">
              <PixelKey size={16} className="text-gold-300" />
              CorridorKey — удаление зелёного фона
            </h1>
            <p className="text-sm text-zinc-400 mt-1 max-w-md">
              Перетащите видео сюда. Нейросеть Robust Video Matting вырежет
              субъекта и заменит фон по выбору.
            </p>
          </div>
      ...
      <OutputDirCard moduleId="corridor_key" moduleLabel="CorridorKey" disabled={phase === "running"} />

      <GlassCard className="text-[11px] text-zinc-500">
        <div className="flex items-center gap-2">
          <PixelDownload size={12} />
          Robust Video Matting (CoreML / CPU). Прозрачный режим пишет
          ProRes 4444 .mov, остальные — H.264 .mp4. Аудио пока не сохраняется.
        </div>
      </GlassCard>
    </div>
  );
